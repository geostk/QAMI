...
imgBlurry(:,:,1) = imgRef;
%todo:1. 51张图像与参考图像的相似度
%todo:2. 取第2大值与最小值对应图像的索引
%todo:3. define a function,make imgRef and img as input args,outputs the reservered cols and rows 
num = length(ssimIndex);
for i = 1:num
    img = double(dicomread(dcmInfo{ssimIndex(i)}));
    tmp = corrcoef(imgRef,img);
    similarity(i) = tmp(2);
end

minIndex = find(similarity == min(similarity));
similarity(similarity == max(similarity)) = 0;
maxIndex = find(similarity == max(similarity));
